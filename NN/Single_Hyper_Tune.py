...
X_train = pad_sequences(list_tokenized_train, maxlen=Para.sentence_length, padding='post', truncating='post')
X_val = pad_sequences(list_tokenized_val, maxlen=Para.sentence_length, padding='post', truncating='post')
...
x = GlobalMaxPool1D()(x)
kernel_initial = initializers.TruncatedNormal()
kernel_regular = regularizers.l2(0.1)
x = Dense(50, activation="relu", kernel_initializer=kernel_initial, kernel_regularizer=kernel_regular)(x)
...
model.fit(X_train, y_train, validation_data=(X_val, y_val), batch_size=Para.batch_size, epochs=Para.epchos,
